...
const playerNames = [
  ...
  "Serge Gnabry", "Kingsley Coman", "Ousmane Dembélé", "Randal Kolo Muani",
  "Gonçalo Ramos", "Milan Škriniar", "Gianluigi Donnarumma", "Mike Maignan",
  "Emiliano Martínez", "Manuel Neuer", "Jan Oblak", "Yann Sommer", "Wojciech Szczęsny",
  "David Raya", "Guglielmo Vicario", "Jeremie Frimpong", "Álex Grimaldo",
  "Jonathan Tah", "Piero Hincapié", "Edmond Tapsoba", "Granit Xhaka",
  ...
  "Kim Min-jae", "Matthijs de Ligt", "Eric Dier", "Nico Schlotterbeck",
  ...
  "Jorginho", "Thomas Partey", "Ben White", "Gabriel Magalhães",
  "Takehiro Tomiyasu", "Oleksandr Zinchenko", "Jurriën Timber", "Jack Grealish",
  "Jérémy Doku", "Mateo Kovačić", "Matheus Nunes", "Manuel Akanji",
  "Nathan Aké", "Josko Gvardiol", "Rico Lewis", "Stefan Ortega", "Harvey Elliott",
  "Curtis Jones", "Wataru Endo", "Joe Gomez", "Ibrahima Konaté",
  ...
  "Mário Rui", "Mathías Olivera", "Amir Rrahmani", "Juan Jesus",
  ...
  "Willy Boly", "Matz Sels", "Riyad Mahrez", "Karim Benzema", "N'Golo Kanté",
  "Sadio Mané", "Neymar", "Roberto Firmino", "Fabinho", "Kalidou Koulibaly",
  "Édouard Mendy", "Ruben Neves", "Aleksandar Mitrović", "Aymeric Laporte",
  "Marcelo Brozović", "Franck Kessié", "Bono", "Sergej Milinković-Savić",
  "Yannick Carrasco", "Pelé", "Diego Maradona", "Ronaldo Nazário", "Johan Cruyff",
  "Zinedine Zidane", "Ronaldinho", "Thierry Henry", "Paolo Maldini", "Cafu",
  "Roberto Carlos", "Franz Beckenbauer", "Xavi", "Andrés Iniesta", "Ruud Gullit",
  "Lothar Matthäus"
];
...
async function getWikiImagesBatch(names: string[]) {
    try {
        const url = `https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(names.join('|'))}&prop=pageimages&redirects=1&format=json&pithumbsize=300&origin=*`;
...
async function run() {
    ...
    let result = `import { Player } from '../types';\n\nexport const PLAYERS: Player[] = [\n`;
    fs.writeFileSync('src/data/players.ts', result);
    
    let idCounter = 1;
    let foundImages = 0;
    
    for (let i = 0; i < playerNames.length; i += 40) {
        const batch = playerNames.slice(i, i + 40);
        const imgMap = await getWikiImagesBatch(batch);
        
        // Check standard
        for (const name of batch) {
            let img = imgMap[name.toLowerCase()];
            if (!img) {
                // Fuzzy match
                const nParts = name.toLowerCase().split(' ');
                const match = Object.keys(imgMap).find(k => nParts.every(part => k.includes(part)));
                if (match) img = imgMap[match];
            }
            ...
            if (img) {
                ...
                if (isGK) {
                    ...
                    pos = ['CB', 'LB', 'RB', 'LWB', 'RWB'][Math.floor(Math.random() * 5)];
                ...
                fs.appendFileSync('src/data/players.ts', '  ' + JSON.stringify(p) + ',\n');
            }
        }
    }
    
    fs.appendFileSync('src/data/players.ts', `];\n\nexport const INITIAL_BUDGET = 500_000_000;\n`);
    console.log(`Found ${foundImages} valid players with images.`);
}
