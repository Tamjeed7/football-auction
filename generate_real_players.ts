...
const cacheFile = 'wiki_cache.json';
    ...
    try {
        ...
        if (pages) {
            ...
            if (firstPage?.thumbnail?.source) {
                cache[name] = firstPage.thumbnail.source;
                return cache[name];
            }
        }
        
        // try with (footballer) appended
        const url2 = `https://en.wikipedia.org/w/api.php?action=query&titles=${encodeURIComponent(name + ' (footballer)')}&prop=pageimages&format=json&pithumbsize=300&origin=*`;
...
async function run() {
    ...
    // Self-delete script after successful execution
    try {
       fs.unlinkSync('generate_real_players.ts');
    } catch (e) {}
}
